<section class="ss_index_one">
    ...
    <div class="container-fluid">
        ...
        <div class="evsun-lang">
            <div class="dropdown">
                <button style="background-color: red !important;" class="dropbtn">
                ...
                <div id="languageSelector" class="dropdown-content">
                    @foreach(config('voyager.multilingual.locales') as $lang)
                        <a class="dropdown-item" href="{{"route"}}"><span class="flag-icon flag-icon-{{$lang == 'en' ? 'gb' : $lang}}"></span> {{ucfirst($lang)}}</a>
                    @endforeach
                </div>
            </div>
        </div>
